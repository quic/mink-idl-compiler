...
use idlc_codegen_c::interface::variable_names::invoke::{BI, BI_NAME, BO, BO_NAME};
    ...
    {BI_NAME}.m_{ident} = {ident}_val;"#
            ...
            Type::SmallStruct(s) => {
                if s.contains_interfaces() {
                    assignments += &format!(
                        r#"
    {BI_NAME}.m_{ident} = {ident}_cpy;"#
                    );
                } else {
                    assignments += &format!(
                        r#"
    {BI_NAME}.m_{ident} = {ident}_ref;"#
                    );
                }
            }
        });
