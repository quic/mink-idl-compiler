...
fn rust_itest1_array() {
    ...
    assert_eq!(
        cpp_wrapper.test_obj_array_in(&input_array),
        Ok(SUCCESS_FLAG)
    );

    let (objs, value) = cpp_wrapper.test_obj_array_out().unwrap();
    assert!(value == SUCCESS_FLAG);
    for obj in objs {
        assert_eq!(cpp_wrapper.test_obj_in(obj.as_ref()), Ok(SUCCESS_FLAG));
    }
}
